...
public class MenuController {
	...
	public String attendance(HttpServletRequest request, HttpSession session, Model model) {
		...
		if(atChk>=1) {
			...
			if(reAt>=1) {
				session.setAttribute("msg","출석 추가 성공");
				//model.addAttribute("msg","출석 추가 성공");
				System.out.println("출석 추가 성공");//성공됨
				
				int monthCount = aService.atCount(user_id, atYear, atMonth);
				if(monthCount==6) {
					session.setAttribute("msg", "축하합니다! 이번달 쿠폰을 발급받으셨습니다.");
					return "/member/coupon";
				}else {}
				
				/*int userCount = aService.userCount(user_id);
				if(userCount==6) {
					session.setAttribute("msg", "누적 출석 10회기념 쿠폰 발급!");
					//model.addAttribute("msg", "누적 출석 10회기념 쿠폰 발급!");
				}else {}*/
				
				/*int allCount = aService.allCount();
				if(allCount==8) {
					session.setAttribute("msg", "전 회원 누적 출석 수 12번째 주인공에게 쿠폰 발급!");
					//model.addAttribute("msg", "전 회원 누적 출석 수 12번째 주인공에게 쿠폰 발급!");
				}else {}*/
